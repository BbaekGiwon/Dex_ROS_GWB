...
def main():
    ...
    parser.add_argument(
        "--start-on-play", action="store_true", help="If present, to true."
    )
    ...
    omni.kit.async_engine.run_coroutine(
        open_stage_async(options.path, options.start_on_play)
    )
...
async def open_stage_async(path: str, start_on_play: bool):
    ...
    async def _open_stage_internal(path):
        ...
        if is_stage_with_session:
            ...
            (success, error) = (
                await layers.get_live_syncing().open_stage_with_live_session_async(path)
            )
        else:
    ...
    if broken_url.scheme == "omniverse":
        ...
        nucleus_connector.connect(
            ...
            on_failed_fn=lambda *_: carb.log_error(
                f"Open stage: Failed to connect to server '{server_url}'."
            ),
        )
    else:
