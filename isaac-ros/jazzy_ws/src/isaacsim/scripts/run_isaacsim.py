...
def update_env_vars(version_to_remove, specified_path_to_remove, env_var_name):
    ...
    for path in env_var_value.split(os.pathsep):
        if not version_to_remove in path and not path.startswith(
            specified_path_to_remove
        ):
...
class IsaacSimLauncherNode(Node):
    ...
    def execute_launch(self):
        ...
        args.install_path = (
            self.get_parameter("install_path").get_parameter_value().string_value
        )
        args.use_internal_libs = (
            self.get_parameter("use_internal_libs").get_parameter_value().bool_value
        )
        args.dds_type = (
            self.get_parameter("dds_type").get_parameter_value().string_value
        )
        args.gui = self.get_parameter("gui").get_parameter_value().string_value
        args.standalone = (
            self.get_parameter("standalone").get_parameter_value().string_value
        )
        args.play_sim_on_start = (
            self.get_parameter("play_sim_on_start").get_parameter_value().bool_value
        )
        args.ros_distro = (
            self.get_parameter("ros_distro").get_parameter_value().string_value
        )
        args.ros_installation_path = (
            self.get_parameter("ros_installation_path")
            .get_parameter_value()
            .string_value
        )
        args.headless = (
            self.get_parameter("headless").get_parameter_value().string_value
        )
        args.custom_args = (
            self.get_parameter("custom_args").get_parameter_value().string_value
        )
        args.exclude_install_path = (
            self.get_parameter("exclude_install_path")
            .get_parameter_value()
            .string_value
        )
        ...
        if args.install_path != "":
            ...
            if version_ge(args.version, "4.2.0") and not version_gt(
                args.version, "2021.2.0"
            ):
                if sys.platform == "win32":
            ...
            elif version_ge(args.version, "2021.2.1") and not version_ge(
                args.version, "2023.1.2"
            ):
                if sys.platform == "win32":
                    filepath_root = os.path.join(
                        home_path,
        ...
        if args.use_internal_libs:
            if sys.platform == "win32":
                ...
                os.environ["LD_LIBRARY_PATH"] = (
                    f"{os.getenv('LD_LIBRARY_PATH')}:{filepath_root}/exts/isaacsim.ros2.bridge/{args.ros_distro}/lib"
                )
                specific_path_to_remove = f"/opt/ros/{args.ros_distro}"
                version_to_remove = "jazzy" if args.ros_distro == "humble" else "humble"
                update_env_vars(
                    version_to_remove, specific_path_to_remove, "LD_LIBRARY_PATH"
                )
                update_env_vars(
                    version_to_remove, specific_path_to_remove, "PYTHONPATH"
                )
        ...
        if args.ros_installation_path:
            ...
            if sys.platform == "win32":
                ...
                ros_paths = [
                    path.strip()
                    for path in args.ros_installation_path.split(",")
                    if path.strip()
                ]
                ...
                for ros_path in ros_paths:
                    ...
                    if ros_path.endswith("setup.bash") or "setup.bash" in ros_path:
                        ...
                        if current_ld_path:
                            os.environ["LD_LIBRARY_PATH"] = (
                                f"{install_path}/lib:{current_ld_path}"
                            )
                        else:
                        ...
                        if current_python_path:
                            os.environ["PYTHONPATH"] = (
                                f"{install_path}/lib/python3/dist-packages:{current_python_path}"
                            )
                        else:
                            os.environ["PYTHONPATH"] = (
                                f"{install_path}/lib/python3/dist-packages"
                            )

        os.environ["RMW_IMPLEMENTATION"] = (
            "rmw_cyclonedds_cpp"
            if args.dds_type == "cyclonedds"
        ...
        if args.standalone != "":
            executable_path = os.path.join(
                filepath_root, "python.sh" if sys.platform != "win32" else "python.bat"
            )
            proc = subprocess.Popen(
                f"{executable_path} {args.standalone}",
                shell=True,
            ...
            if args.gui != "":
                ...
                file_arg = (
                    os.path.join(script_dir, "open_isaacsim_stage.py")
                    + f" --path {args.gui} {play_sim_on_start_arg}"
                )
                executable_command += f" --exec '{file_arg}'"

            proc = subprocess.Popen(
                executable_command, shell=True, start_new_session=True
            )
            subprocesses.append(proc.pid)
